...
class AmddDatabaseJoomla extends AmddDatabase
{
    ...
    public function updateDatabase($stream)
    {
        ...
        $debuglevel = (substr(JVERSION, 0, 3) === '1.5') ? $config->getValue('config.debug') : $config->get('debug');
        ...
        while (!feof($stream)) {
            ...
            // @todo use fast quote implementation instead of SQL's real escape
            $insert_sql .= '('
                . $db->quote($ua) . ','
                . $db->quote($group, false) . ','
                . $db->quote($data)
                . ')';
