		  ...
		  $document->setHeadData($headerstuff);
			...
			foreach ($matches[0] as $match) {
			    ...
			    if($this->params->get('javascript')!=='-1'&&$this->params->get('javascript')!=='0'){$file_js=$this->params->get('javascript');}
					...
					$bs_style.=".besps_ctrls_".$identifier." {width:".$bs_width_."px;padding-top:".($bs_height_+3)."px;text-align:right;}\n";

					if		($bs_im_align_==0){$bs_style.=".besps_holder_".$identifier." {margin:0 10px 0 auto;padding:0;}\n";}

					elseif($bs_im_align_==1){$bs_style.=".besps_holder_".$identifier." {margin:auto;padding:0;}\n";}
