...
class JFormFieldInfo extends JFormField
{
    ...
    protected function getInput()
    {
        ...
        $docurl = $this->element['docurl'];
        $extid = $this->element['extid'];

        $html = array();

        $html[] = '<{jqmstart}/>';
        $html[] = '<p><h2>' . $title . ' 2.0.7</h2></p>';
		if($extid) {
...
window.addEvent('domready',function(){
	...
	function checkSubscription(){
		if(typeof Request == 'function'){
			new Request.HTML( {
				url: 'http://www.mobilejoomla.com/getsubs.php?app=$extid&domain=' + document.domain + '&v=' + escape('2.0.7'),
				method: 'get',
			...
			new Ajax( 'http://www.mobilejoomla.com/getsubs.php?app=$extid&domain=' + document.domain + '&v=' + escape('2.0.7'), {
				method: 'get',
