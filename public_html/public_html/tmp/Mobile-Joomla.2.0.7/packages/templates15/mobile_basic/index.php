...
$templateDetails = simplexml_load_file(dirname(__FILE__).'/templateDetails.xml');
foreach($templateDetails->params->param as $xmlParam)
{
	if((string)$xmlParam['name'])
		$this->params->def((string)$xmlParam['name'], (string)$xmlParam['default']);
}
...
if($mj->isHome() && !JRequest::getInt('limitstart'))
{
	$logo = $this->params->get('logo');
	if($logo == '' || $logo == '-') $this->params->set('title', 1);

	$ress_attrs = $mj->getParam('ress_optimize') ? ' ress-nowrap ress-fullwidth' : '';
	switch($this->params->get('title'))
	{
		case 2:
			$page_header = '<img src="'.JUri::base(true).$logo.'"'.$ress_attrs.' alt="">';
			break;
		case 3:
			$page_header = '<img src="'.JUri::base(true).$logo.'"'.$ress_attrs.' alt=""> '.$app->getCfg('sitename');
			break;
		case 1:
		default:
...
?>
	<div class="footer" data-role="footer"<?php echo $attrs; ?>><?php
		...
		$mj->generator && $mj->generator->showFooter();
