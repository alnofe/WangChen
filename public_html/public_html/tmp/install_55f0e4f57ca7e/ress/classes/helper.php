...
class Ressio_Helper
{
    ...
    public static function setHeader($line, $override = true, $http_response_code = null)
    {
        if (strpos($line, ':') === false) {
            ...
            if ($override || !isset($headers[$prop])) {
                self::$headers[$prop] = $line;
            } else {
                if (is_array($headers[$prop])) {
                    self::$headers[$prop][] = $line;
                } else {
                    self::$headers[$prop] = array($headers[$prop], $line);
                }
            }
        }
    }
    ...
    public static function sendHeaders()
    {
        ...
        if (function_exists('Ressio_FuncOverride_sendHeaders')) {
            Ressio_FuncOverride_sendHeaders(self::$status, self::$headers);
        } else {
            header(self::$status);
