...
class Ressio_UrlRewriter
{
    ...
    private $base_host;
    /** @var string */
    private $base_path; // base path with both leading and trailing slash
    ...
    public function minify($url)
    {
        ...
        if (!empty($normal_url) || $parsed['host'] !== $this->base_host) {
            $normal_url .= '//' . $parsed['host'];
        }

        if (empty($normal_url) && strpos($parsed['path'], $this->base_path) === 0) {
