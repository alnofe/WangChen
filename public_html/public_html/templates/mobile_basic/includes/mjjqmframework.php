...
/*        if (version_compare(JVERSION, '1.6', '<')) {
            ...
            $dispatcher->attach(array('event' => '', 'handler' => array($this, '')));
        }*/
...
//		$this->params->set('load_external', 1); // for correct work of combiner
     ...
     * @param $headerstuff array
     */
    private function generateHead(&$headerstuff)
    {
        ...
        $document->setBuffer($head, 'mjhead');
    }
...
//				$cdn = ($protocol==='https:') ? 'aspnetcdn.com' : 'jquery.com';
    ...
    {
        $base = dirname($url) . '/';
        return preg_replace('#\burl\(\s*?(?:["\'](?!/|\w+?:)|(?![/"\']|\w+?:))#s', '\\0' . $this->baseurl . $base, $content);
    }
    ...
    private function combinerGet($type)
    {
        ...
        /*if(!JDEBUG && $this->params->get('combine'))
        {
            ...
            $cached_url  = $this->baseurl . '/' . ltrim(str_replace(JPATH_ROOT, '', $cached_file), '/\\');
    ...
    private function loadIcons()
    {
        ...
        $icons = array(
            ...
            'touch-icon-180x180.png' => 'rel="apple-touch-icon" sizes="180x180"', // iPhone 6 Plus, iOS 8
        ...
        $startups = array(
            ...
            'touch-startup-image-750x1294.png' => 'rel="apple-touch-startup-image" media="(device-width: 375px) and (device-height: 667px) and (-webkit-device-pixel-ratio: 2)"', // iPhone 6
            'touch-startup-image-1242x2148.png' => 'rel="apple-touch-startup-image" media="(device-width: 414px) and (device-height: 736px) and (-webkit-device-pixel-ratio: 3)"', // iPhone 6 Plus
...
//		$mobileinit[] = 'jQuery.mobile.loadingMessage=false;';
        ...
        $custom_js = $this->path . '/js/custom.js';
        if (is_file($custom_js) && filesize($custom_js))
